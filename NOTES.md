...
- Next pending keyword: "best debt payoff apps compared" (app-comparisons
  pillar). Covers Undebt.it (free, manual, no bank sync), YNAB
  (budgeting-first, debt as a symptom of not tracking spending), Debt
  Payoff Planner (dedicated mobile countdown), Rocket Money (not a payoff
  app — a subscription/bill leak-finder to pair with one), and Empower
  (free all-in-one dashboard with debt as one feature among several).
  Checked current pricing via web search rather than relying on
  training-data figures, same as the investing-apps post.
- Followed the normal `post/<slug>` branch + PR flow (PR #35).
  `pinApproved`/`depthReviewed` left at schema defaults for human review.
  Marked the keyword `published` via a single targeted string replace,
  confirmed as a one-line diff.
- `npm run build` succeeds and the new route generates. Running it
  locally also picked up `public/pinterest-feed-saving.xml`, which had
  gone stale after PR #33 merged (same drift pattern as the
  app-comparisons feed on 2026-08-30) — included as a side effect of
  this session's build, not a separate fix.

### Confirmed: PR #34's concurrency-group fix for pin-preview/x-draft works
Watched both workflows on this PR directly: `pin-preview.yml` ran to
completion first, `x-draft.yml` then started only after — the shared
`concurrency` group serialized them as intended, no non-fast-forward
push rejection this time.

### Found: x-draft.yml's "Pull latest before committing" step fails independently
`x-draft.yml` still failed on this PR, but not from the race — its
"Generate draft(s) for new posts" step succeeded, then "Pull latest
before committing" (`git pull --rebase origin <branch>`) failed right
after, before anything was committed. Couldn't confirm the exact git
error (log host is outside the network allowlist, same limitation
logged 2026-08-27/08-30). Best guess: the generate step leaves
`content-pipeline/x-drafts.md` modified-but-uncommitted in the working
tree, and `git pull --rebase` refuses to rebase over local uncommitted
changes — which would make this fail on every PR going forward,
independent of whether there's anything new to actually pull. Worked
around it this session by running `node scripts/generate-x-drafts.mjs
--all` locally (after rebasing onto pin-preview's already-pushed
commit) and pushing directly, same pattern as the earlier pin/hero
workaround.

### Open items
- PR #35 needs review/merge (post, pin, hero, x-draft all ready; pin was
  generated by CI, x-draft was committed manually due to the failure
  above).
- x-draft.yml's rebase-step failure — not fixed, will likely recur on
  the next post's PR until the workflow either commits its own working-
  tree changes before pulling, or drops the redundant pull now that the
  concurrency group already prevents the original race.
- Pinterest feed staleness after merge (carried over from 2026-08-30) —
  still not fixed at the CI level; a post-merge workflow to rebuild is
  still the likely fix.
- Whether `ANTHROPIC_API_KEY` was ever added as a repo secret for the
  now-expired auto-publish trial — still unconfirmed (carried over).
